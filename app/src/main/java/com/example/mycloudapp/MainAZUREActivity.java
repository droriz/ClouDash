...
public class MainAZUREActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CloudProviderData azureProvideData = (CloudProviderData)intent.getSerializableExtra("azure_data");
        Random rnd = new Random();
        for(int i = 0; i<azureProvideData.regions.length; i++){
            SliceValue slice = new SliceValue((int)(azureProvideData.regions[i].balance),Color.rgb(rnd.nextInt(256),rnd.nextInt(256),rnd.nextInt(256)) );
            slice.setLabel(azureProvideData.regions[i].name);
        ...
        for(int i = 0; i<azureProvideData.regions.length; i++){
            totalCapacityBalance += azureProvideData.regions[i].capacityBalance;
            totalNetworkBalanceBalance += Float.parseFloat(azureProvideData.regions[i].networkBalance);
            totalComputeBalance += Float.parseFloat(azureProvideData.regions[i].computeBalance);
        }
